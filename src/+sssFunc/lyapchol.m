    ...
    case 'adi'
        ...
        if (nargout>1 && Opts.infoLyap == 1) || (nargout == 2 && Opts.infoLyap == 0) || nargout == 3
            if sys.isSym && ~any(size(sys.B)-size(sys.C')) && norm(full(sys.B-sys.C'))==0
                ...
                data.Info_R = Rout;
                if Opts.rctol ~= 0 && Rout.rc(end)>Opts.rctol
                    warning(['Maximum number of ADI iterations reached (maxiter = ',num2str(Opts.maxiter,'%d'),...
                        '). rctol is not satisfied for R: ',num2str(Rout.rc(end),'%d'),' > rctol (',num2str(Opts.rctol,'%d'),').']);
                elseif Opts.rctol ~= 0 && Rout.res(end)>Opts.restol
                     warning(['Maximum number of ADI iterations reached (maxiter = ',num2str(Opts.maxiter,'%d'),...
                        '). restol is not satisfied for R: ',num2str(Rout.res(end),'%d'),' > rctol (',num2str(Opts.restol,'%d'),').']);
                end
            end
