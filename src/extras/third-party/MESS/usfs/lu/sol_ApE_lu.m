function X=sol_ApE_lu(eqn, opts,opA,p,opE,C,opC)
...
if ~isfield(eqn,'p') || isempty(eqn.p)
    newlu=true;
elseif p==eqn.p
    newlu=false;
else
...
if newlu
    ...
    [eqn.LP_LCp, eqn.LP_UCp, eqn.LP_aCp, eqn.LP_oCp, eqn.LP_SCp] = lu(eqn.A_+p*eqn.E_,'vector'); %[L,U,a,o,S]=lu(A-pI),'vector');
    eqn.p=p;
end
  ...
  case 'N'
      ...
      switch opC
          ...
          case 'N'
              ...
              X(eqn.LP_oCp,:) = eqn.LP_UCp\(eqn.LP_LCp\(eqn.LP_SCp(:,eqn.LP_aCp)\C));
%               X = (eqn.A_+p*eqn.E_)\C;
  ...
  case 'T'
      ...
      switch opC
          ...
          case 'N'
              ...
              X = (eqn.LP_SCp(:,eqn.LP_aCp)).'\(eqn.LP_LCp.'\(eqn.LP_UCp.'\(C(eqn.LP_oCp,:))));
%               X = (eqn.A_'+p*eqn.E_)\C;
