function X = sol_ApE_dae3_so(eqn, opts, opA, p, opE, B, opB)
      ...
      switch opE
        ...
        case 'N'
          ...
          switch opB
            ...
            case 'N'
              x13 = [eqn.K_-p*eqn.D_-p^2*eqn.M_, eqn.G_'; eqn.G_,zeros(np,np)]\[B(nv+1:2*nv,:)-eqn.D_*B(1:nv,:)-p*eqn.M_*B(1:nv,:);B(2*nv+1:end,:)];
      ...
      switch opE
        ...
        case 'N'
            ...
            switch opB
                %implement solve (A_+p*E_)*X=B
                case 'N'
                  x23 = [eqn.K_'-p*eqn.D_'-p^2*eqn.M_, eqn.G_'; eqn.G_,zeros(np,np)]\[B(1:nv,:)-p*B(nv+1:2*nv,:);-B(2*nv+1:end,:)];
                  X = [(B(1:nv,:)-eqn.K_'*x23(1:nv,:)-eqn.G_'*x23(nv+1:end,:))./p; x23(1:nv,:); x23(nv+1:end,:)];
                  %X = (A' + p * E) \ B;
