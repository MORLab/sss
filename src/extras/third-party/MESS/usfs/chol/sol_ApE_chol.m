function X=sol_ApE_chol(eqn, opts,opA,p,opE,C,opC)
...
%% check if new chol is necessary
...
%% from lyapack
if newchol
    [eqn.LP_Up,t] = chol(-(eqn.A_+p*eqn.E_));
    eqn.p=p;
    
    if t~=0
      error('The matrix (A+pE) is not (numerically) negative definite!');
    end
end
